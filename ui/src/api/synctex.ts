...
export const synctexApi = {
  ...
  synctexEdit: (page: number, x: number, y: number, branch?: string, review = false) =>
    json<SourceLocation>(
      '/api/synctex/edit' +
        qs({ page, x: x.toFixed(2), y: y.toFixed(2), branch, review }),
    ),
  ...
  synctexView: (path: string, line: number, branch?: string, review = false) =>
    json<SourceView>('/api/synctex/view' + qs({ path, line, branch, review })),
}
